...
namespace MobiFlight.xplane
{
    public class XplaneCache : XplaneCacheInterface
    {
        ...
        public bool Connect()
        {
            if (Connector == null)
            {
                Connector = new XPlaneConnector.XPlaneConnector("192.168.0.200"); // fixed ip for now
               // Connector = new XPlaneConnector.XPlaneConnector();

                Log.Instance.log($"Connector to pc created", LogSeverity.Debug);
            }
        ...
        {
           // Log.Instance.log($"Checking if xplane is connected", LogSeverity.Debug);
            if (!_connected) Log.Instance.log("X plane is not connected", LogSeverity.Debug);
            return _connected;
        }
        ...
        {
            SubscribedDataRefs.Clear();
            Log.Instance.log("Starting", LogSeverity.Debug);
            Connector?.Start();
        }
